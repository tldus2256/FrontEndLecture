    ...
    for (let i = 0; i < powerArr.length; i++) {
        if ((i +1)% 5 == 0)
            ...
            if (i==powerArr.length - 1)
                resultStr += powerArr[i];              // 맨 마지막 요소 뒤에는 , 를 붙이지 않겠다
            else
...
Grown-ups never understand anything by themselves, and it is tiresome for children to be always and forever explaining 
things to the.;
*/
...
function ans2() {
    ...
    document.getElementById('result2').innerHTML = `${search} 개수: ${count}`;
}
...
function ans3() {
    ...
    // priceValueArr 와 newPrices는 같은 배열
    let priceValueArr = [];
